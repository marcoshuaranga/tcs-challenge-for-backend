...
- [ ] 1.1 Add `@asteasolutions/zod-to-openapi` to `core/contracts/package.json`;
      run `pnpm install`
- [ ] 1.2 Call `extendZodWithOpenApi(z)` at the top of `core/contracts/src/index.ts`
      (must run before any schema is defined)
- [ ] 1.3 Add `.openapi({ ref: '...' })` metadata to `OrderCreateSchema`,
      `OrderResponseSchema`, and error envelope schemas
- [ ] 1.4 Run `pnpm run lint` and `pnpm dlx prettier --write .` on `core/contracts`
- [ ] 1.5 Commit: `feat(contracts): annotate Zod schemas with zod-to-openapi metadata`

## 2. apps/api-docs — setup + implementation

- [ ] 2.1 Create `apps/api-docs/package.json` with name
...
- [ ] 2.2 Create `apps/api-docs/tsconfig.json` extending `../../tsconfig.node.json`
- [ ] 2.3 Add `apps/api-docs` to `pnpm-workspace.yaml`; run `pnpm install`
- [ ] 2.4 Implement `apps/api-docs/src/app.ts`: build OpenAPI document from
      `core/contracts` schemas using `OpenAPIRegistry` + `OpenAPIGenerator`;
      mount `GET /openapi.json` and Scalar UI `GET /` via Hono
- [ ] 2.5 Create `apps/api-docs/src/index.ts`: start server with `@hono/node-server`
      on `PORT` (default 3001)
- [ ] 2.6 Verify dev server: `GET /openapi.json` returns valid JSON;
      `GET /` renders Scalar UI in browser
- [ ] 2.7 Run `pnpm run lint` and `pnpm dlx prettier --write .` on `apps/api-docs`
