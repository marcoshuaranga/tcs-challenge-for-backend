...
DynamoDB-backed implementation of `OrderRepositoryPort` (`findById`, `save`, `listAll`).
Single-table design: `PK=ORDER#<id>`, `SK=METADATA`, `GSI1PK=ORDERS`, `GSI1SK=<createdAt>#<id>`.
Supports `DDB_ENDPOINT` for local development and integration tests.
